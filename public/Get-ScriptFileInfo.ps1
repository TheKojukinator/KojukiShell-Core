Function Get-ScriptFileInfo {
    ...
    process {
        try {
            # try and get the calling script from the callstack, it should be the last item that has ScriptName set and FunctionName is "<ScriptBlock>"
            $scriptName = Get-PSCallStack | Where-Object { $null -ne $PSItem.ScriptName -and $PSItem.FunctionName -eq "<ScriptBlock>" } | Select-Object -Last 1 | Select-Object -ExpandProperty ScriptName
